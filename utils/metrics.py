...
class ClassificationMetric(Metric):
    ...
    def __call__(self,
        ...
        self._all_losses = torch.cat([self._all_losses, losses.to(self._all_losses.device)], dim=0)#按行拼接
...
class MaskModelMetric(Metric):
    ...
    def __call__(self,
        ...
        self._all_losses = torch.cat([self._all_losses, losses.to(self._all_losses.device)], dim=1)#按列拼接
        predictions = logits.argmax(-1).to(self._all_predictions.device)
        self._all_predictions = torch.cat([self._all_predictions, predictions], dim=0)
        self._all_gold_labels = torch.cat([self._all_gold_labels, gold_labels.to(self._all_gold_labels.device)],dim=0)
