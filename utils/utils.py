...
def convert_batch_to_mlm_input_dict(batch:Tuple[torch.Tensor], max_ngram_length:int):
    '''
    :param model_type: model type for example, 'bert'
    :param batch: tuple, contains 3 element, batch[0]: input ids
            batch[2]:attention_mask, batch[1]: output ids
    :return:
    '''

    inputs = {}
    
    inputs['input_ids'] = batch[0].view([-1,max_ngram_length])
    inputs['attention_mask'] = batch[1].view([-1,max_ngram_length])
    # for distilbert and dcnn, token_type_ids is unnecessary
    # if model_type != 'distilbert' and model_type != 'dcnn':
    #     inputs['token_type_ids'] = prepare_token_type_ids(batch[2], model_type)
